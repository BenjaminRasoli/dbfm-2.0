...
async function fetchFromDBFM(type: string, id: string, endpoint: string) {
...
async function getSingleMovieOrTv(id: string, type: string) {
  try {
    const [mediaData, videoData, actorsData, reviewsData, whereToWatchData] =
      await Promise.all([
        fetchFromDBFM(type, id, ""),
        fetchFromDBFM(type, id, "/videos"),
        fetchFromDBFM(
          type,
          id,
          type === "movie" ? "/credits" : "/aggregate_credits"
        ),
        fetchFromDBFM(type, id, "/reviews"),
        fetchFromDBFM(type, id, "/watch/providers"),
      ]);
...
export default async function Page({
  ...
  try {
    const { mediaData, videoData, actorsData, reviewsData, whereToWatch } =
      await getSingleMovieOrTv(slug, "movie");

    if (!mediaData) return <NotFound />;
